...
const PHP_PDF_SCRIPT = path.join(__dirname, '..', '..', 'php-desercion', 'generar_desercion.php');
const PHP_EXCEL_SCRIPT = path.join(__dirname, '..', '..', 'php-desercion', 'generar_desercion_excel.php');
...
async function generatePdfWithPhp(idAlumno) {
  const args = [];
  if (idAlumno) args.push(`id_alumno=${idAlumno}`);
  return runPhpScript(PHP_PDF_SCRIPT, args);
}

async function generateExcelWithPhp(idAlumno) {
  const args = [];
  if (idAlumno) args.push(`id_alumno=${idAlumno}`);
  return runPhpScript(PHP_EXCEL_SCRIPT, args);
}
